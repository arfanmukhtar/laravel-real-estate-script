...
class MainController extends Controller
{
    ...
    public function detail($slug)
    {
        ...
        $post_detail = DB::table('post_details')->where('post_id', $post->id)->first();
        $custom_fields = DB::table('custom_fields')->get();
        $custom_fields_array = array();
        foreach($custom_fields as $c) { 
            $custom_fields_array["$c->id"] = $c->name;
        }
        $title = $post->title;
        ...
        return view('posts::detail', compact('post', 'post_detail','custom_fields_array', 'relatedAds', 'images', 'title', 'canonicalUrl'));
    }
