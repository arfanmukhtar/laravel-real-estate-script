                    ...
                    <div class="inner inner-box ads-details-wrapper">
                        ...
                        <!--item-slider-->
                         ...
                         <table width="100%" class="table table-bordered text-center table-blocks">
                            <tbody>
                                <tr>
                                    @if(!empty($post_detail) and !empty($post_detail->build_year))
                                    <td width="25%">
                                        <span class="fa fa-calendar"></span>
                                        <p><a href="search?l=&q=2015" title="Year 2015 Cars for sale in Pakistan">{{$post_detail->build_year}}</a>
                                        </p>
                                    </td>
                                    @endif
                                    @if(!empty($post_detail) and !empty($post_detail->bedrooms))
                                    <td width="25%">
                                        <span><i class="fa fa-bed"></i></span>
                                        <p>{{$post_detail->bedrooms}}</p>
                                    </td>
                                    @endif
                                    @if(!empty($post_detail) and !empty($post_detail->bathrooms))
                                    <td width="25%">
                                        <span><i class="fa fa-bath"></i></span>
                                        <p>{{$post_detail->bathrooms}}</p>
                                    </td>
                                    @endif
                                    @if(!empty($post_detail) and !empty($post_detail->area_size))
                                    <td width="25%">
                                        <span><i class="fa fa-window-close"></i></span>
                                        <p>{{$post_detail->area_size}} {{$post_detail->area_size_type}}</p>
                                    </td>
                                    @endif
                                </tr>
                            </tbody>
                        ...
                        <ul class="list-unstyled ul-featured clearfix" id="scroll_car_detail">
                            ...
                            <?php  $user_slug = Str::slug( $post->user->id . " " . $post->user->name);  ?>
                            <li><a href="{{$user_slug}}">{{$post->contact_name}}</a></li>

                            @if(!empty($post->custom_data))
                            <?php $fields = json_decode($post->custom_data , true); ?>
                                 @foreach($fields  as $key=>$value)
                                    @if(!is_array($value))
                                   <li class="ad-data">{{ $custom_fields_array[$key] }}</li>
                                  <li>{{$value}}</li>
                                    @endif
                                 @endforeach
                        ...
                        @if(!empty($post->custom_data))
                            <?php $fields = json_decode($post->custom_data , true); ?>
                                  @foreach($fields  as $key=>$value) 
                                    <h2 class="ad-detail-heading mt30">{{$custom_fields_array[$key]}}</h2>
                                
                                    @if(is_array($value))
                                    
                                        <ul class="list-unstyled car-feature-list nomargin">
                                            @foreach($value as $v)
                                            <li><i style="color:green" class="fa fa-check"></i>&nbsp; {{$v}}</li>
                                            @endforeach
                                        </ul> 
                                    @endif
                                 @endforeach
                ...
                <div class="col-md-3 col-sm-6">
                    ...
                    @endif
                        ...
                        <div class="card-content">
                            ...
                            <div class="generic-gray"><i class="fa fa-bed"></i> &nbsp;4 &nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;<i class="fa fa-bath"></i> &nbsp; 4</div>
