        ...
        <div class="col-md-12 no-padding photobox is-featured">
            ...
            <div class="add-private-tabimage"> 
                ...
                @if(!empty($post->photos))
                 <div class="attach-photos">
                        <i class="fa fa-picture-o" aria-hidden="true"></i>  {{count($post->photos)}}
                </div>
                @endif
            </div>
            <!--/.photobox-->
            ...
            <div class="add-desc-box">
                ...
                <div class="ads-details">
                    ...
                    @endif
                    <div> </b> {{$catName}} </b>&nbsp;&nbsp; &nbsp;&nbsp;<i class="fa fa-bed"></i> &nbsp; {{$post->post_detail->bedrooms ?? 'N/A'}} &nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;<i class="fa fa-bath"></i> &nbsp; {{$post->post_detail->bathrooms ?? 'N/A'}} &nbsp;&nbsp;&nbsp;&nbsp; {{ number_format($post->post_detail->area_size) . " " . ucfirst(str_replace("_", ". ", $post->post_detail->area_size_type))  ?? 'N/A'}} </div>
                    <div class="city-name"> <span class="item-location"><br><i class="fa fa-map-marker-alt"></i> </span>  {!!$cityName!!}</div>
                    <div class="search-bottom clearfix">
                        <div class="pull-left dated">Added: {{time_elapsed_string($post->created_at)}} {{ trans('search.by') }} {{$post->contact_name}}</div>
                    </div>
                </div>
            </div>
            <!--/.add-desc-box-->
